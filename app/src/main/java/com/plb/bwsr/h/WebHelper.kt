...
class WebHelper(val callback: WebCallback) {
            ...
            override fun onPageFinished(view: WebView, url: String) {
                if (url.isNotEmpty() && isPageLoad) {
                    ...
                    if (uri.isNotEmpty() && uri != "about:blank") {
                        PLBDBManager.instance.pageHistoryDao()
                            .addPage(PLBHistory(uri, title, System.currentTimeMillis()))
    ...
    fun addBookMar(url: String, title: String) {
        if (url.isNotEmpty()) {
            val item = PLBDBManager.instance.pageMarkDao().getByUrl(url)
            if (item == null) {
                PLBDBManager.instance.pageMarkDao()
                    .addPage(PLBMark(url, title, System.currentTimeMillis()))
            }
        }
    }
